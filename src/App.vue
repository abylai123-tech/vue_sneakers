...
const fetchFavorites = async () => {
    try {
      const { data: favorites } = await axios.get(`https://6c2b072e91f77630.mokky.dev/favorites`)
...
const addToFavorite = async (item) => {
    try { 
      ...
      const { data } = await axios.post(`https://6c2b072e91f77630.mokky.dev/favorites`, obj);
      
      item.favoriteId = data.id;
      } else {
        item.isFavorite = false;
        await axios.delete(`https://6c2b072e91f77630.mokky.dev/favorites/${item.favoriteId}`);
...
const fetchItems = async () => {
  try {
    ...
    const { data } = await axios.get(
      `https://6c2b072e91f77630.mokky.dev/items`,
      {
        params 
      }
    )
